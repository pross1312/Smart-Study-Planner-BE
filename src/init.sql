...
CREATE TABLE "pomodoro_setting"(
    id BIGSERIAL NOT NULL UNIQUE PRIMARY KEY,
    user_id BIGSERIAL NOT NULL,
    pomodoro_time INT8 CHECK(pomodoro_time > 0) DEFAULT 25*60,
    break_time INT8 CHECK(break_time > 0) DEFAULT 5*60,
    long_break_time INT8 CHECK(long_break_time > 0) DEFAULT 15*60
);
